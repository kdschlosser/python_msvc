        ...
        for j, pattern in enumerate(GLOBAL_SELECTION_PATTERNS):
            for config in BUILD_CONFIGURATIONS:
                ...
                if (
                    ...
                    line == build_pattern_2
                ):
                    template = GLOBAL_SELECTION_BUILD_TEMPLATE.format(
                        config=config
                    )
                    if template not in sln:
                        lines.insert(count + i, template)
                        count += 1
                elif (
                    fnmatch.fnmatch(line, pattern_1) or
                    fnmatch.fnmatch(line, pattern_2)
                ):
                    ...
                    if template not in sln:
                        lines.insert(count + i + 1, template)
                        count += 1
...
def update_vs_project(env, path):
    ...
    def iter_node(old_node):
        ...
        if old_node.text is not None and 'x64' in old_node.text:
            return None
...
GLOBAL_SELECTION_BUILD_PATTERN = '		{config}|{arch} = {config}|{arch}'
GLOBAL_SELECTION_BUILD_TEMPLATE = '		{config}|x64 = {config}|x64'

GLOBAL_SELECTION_PATTERNS = (
    '		{{*}}.{config}|{arch}.ActiveCfg = {config}|{arch}',
    '		{{*}}.{config}|{arch}.Build.0 = {config}|{arch}',
)
